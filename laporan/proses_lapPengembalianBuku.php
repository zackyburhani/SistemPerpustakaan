...
$pdf->Cell(3,0.5,'No Pengembalian',1,0,'C');
$pdf->Cell(5,0.5,'Nama',1,0,'C');
$pdf->Cell(4,0.5,'Tanggal Kembali',1,0,'C');
$pdf->Cell(6,0.5,'Judul Buku',1,0,'C');
$pdf->Cell(1.5,0.5,'Jumlah',1,1,'C');


$pdf->SetFont('Tahoma','',9);
$query = mysqli_query($koneksi,"SELECT tb_pengembalian.no_pengembalian,nama_anggota,tb_pengembalian.tgl_kembali,judul_buku,detil_kembali.jml_kembali FROM tb_pengembalian,tb_anggota,tb_copybuku,tb_buku,detil_kembali,tb_peminjaman WHERE tb_pengembalian.no_peminjaman = tb_peminjaman.no_peminjaman AND tb_peminjaman.no_anggota = tb_anggota.no_anggota AND tb_copybuku.no_buku = tb_buku.no_buku AND detil_kembali.no_pengembalian = tb_pengembalian.no_pengembalian AND detil_kembali.no_copy = tb_copybuku.no_copy AND tb_pengembalian.tgl_kembali BETWEEN '$Dari' AND '$Sampai' ORDER BY tb_pengembalian.tgl_kembali asc");
...
while ($row = mysqli_fetch_array($query)){
  ...
  $pdf->Cell(1.5,0.5,$row['jml_kembali'],1,1,'C'); 
   ...
   $pdf->Cell(18,0.6,'Total Buku Yang Sudah Kembali:',1,0,'C');
